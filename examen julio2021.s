...
ordenar_ciudades_alfabeticamente:
	...
	lw	$s1, num_ciudades	#$s1 = g
	li	$s0, 0		#finish = false
B4_1:	bnez    $s0, B4_7
        mul     $t0, $s1, 10
        div     $s1, $t0, 13
        bge     $s1, 1, B4_2
...
seleccionar_ciudades_por_pais:
	...
	sw	$s0, 0($sp) 
        ...
        move	$s0, $a0	#pais
        move	$s1, $a1	#Ciudad* array_destino
        
        jal	strlen
        move	$s2, $v0	#$s2 = longitud_pais
        
	li	$s3, 0		#int num_copiadas = 0
	li	$s6, 0		#i
for_scp:
	lw 	$t0, num_ciudades		        
	bge	$s6, $t0, fin_for_scp
	li	$s4, 0		#copiar = true
	li	$s5, 0		#j = 0
for_scp2:
	bge	$s5, $s2, fin_for_scp2
	
	la	$t0, ciudades
	mul 	$t1, $s6, 40
	addu	$t0, $t0, $t1	#&ciudades[i]
	addu	$t0, $t0, 15	#ciudades[i].nombre_pais
	addu	$t0, $t0, $s5
	lb	$t0, 0($t0)
	

	addu	$t3, $s5, $s0	#&pais[j]
	lb	$t3, 0($t3)
if:
	beq	$t0, $t3, fin_if
	li	$s4, 1
